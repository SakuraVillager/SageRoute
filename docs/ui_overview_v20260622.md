...
| `_FigureCard` | 872-1098 | 单个人物卡片组件: 图片区(圆角) + 品牌衍生色朝代标签 + 书签 + 底部统计+查看 |
...
**步骤文件**: `lib/views/create_route_wizard/steps/step1_figure.dart`、`step3_map.dart`

3 步全屏创建路线流程：行程规划 → 选择人物 → 路线规划。最终步骤使用单个 AMapWidget，同时完成地点编辑、交通方式选择和路线实时预览。
...
| Header | 返回按钮(圆圈箭头) + `X / 3` 标注 + 步骤标题 |
| 进度条 | 根据当前步骤显示三分之一、三分之二和完成状态 |
| PopScope | `canPop: _currentStep == 1` — 步骤 2/3 时系统返回先回退到上一步 |
| 底部按钮 | 步骤 2 在向导底栏显示 `下一步`；步骤 3 的 `保存行程` 位于路线时间线面板底部；两个按钮视觉高度均为 38.4px，并保留至少 48px 触控区域 |
| 保存门槛 | 少于 2 个地点或正在规划时禁用；规划成功直接保存；规划失败时弹出“继续调整 / 仍然保存”确认框 |
| 归档动画 | 步骤 3 点击保存后: 半透明遮罩覆盖层 + 缩放对勾图标 + "行程已保存" 文字 → 1.8s 后 `pop(draft)` 返回 |
...
### Step 3 — 路线规划 (step3_map.dart)

| 代码 | 功能 |
|------|------|
| 单地图结构 | 唯一的 AMapWidget 同时显示候选地点、已选地点顺序 Marker 和路线 Polyline，避免两个平台地图视图切页 |
| 交通方式 | 地点时间线的每个相邻路段可独立选择 `TransportType.driving` 驾车或 `TransportType.walking` 步行，地图顶部不再放置统一选择器 |
| 添加地点 | 底部面板右上角打开 AddPlacePage；搜索与人物相关主题筛选仍在弹窗内部完成 |
| 可拖拽面板 | 在 32% / 56% / 78% 高度间吸附；紧凑状态显示地点序列，展开状态使用“编号地点 + 虚线连接 + 分段交通信息”时间线，并支持拖拽排序和删除 |
| 实时规划 | 至少 2 个地点后自动规划；地点增删、排序和任一路段交通方式变化均触发 240ms 防抖重算，各段结果合并为一条 Polyline |
| 并发保护 | RoutePreviewCoordinator 使用 generation 丢弃过期异步结果，确保快速编辑时只渲染最新路线 |
| 路线状态 | 面板展示地点不足、规划中、规划成功、规划失败；底部固定区展示总里程、总时长和保存入口，每个路段展示自身距离与耗时 |
| 地图缩放 | 成功时适配 Polyline 范围；失败或地点不足时适配当前 Marker，不清空用户地点 |
