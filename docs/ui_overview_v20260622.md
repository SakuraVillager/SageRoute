...
4 步全屏创建路线流程（2026-06-22 更新：Step 4 地图页恢复底部面板拖拽和已选地点排序；顶部栏支持隐藏/显示；Step 1 日历恢复原始大小并上移完成按钮）。
...
| 代码         | 功能                                                |
|--------------|-----------------------------------------------------|
| AMapWidget   | 高德地图底图；Marker 使用独立 `_markerPlaces` 层维护，排序时保持地图标记静止 |
| 路线规划     | 添加/删除/排序后按左侧已选地点从上到下的顺序重新规划并绘制 Polyline |
| 可拖拽面板   | 从底部弹出，拖动顶部拉手可在 16% / 26% / 38% / 50% / 64% 多档高度间吸附 |
| 左侧已选地点 | 带序号 + 拖拽手柄排序(ReorderableListView) + 删除按钮 |
| 右侧可用地点 | 点击添加，按名字排序                                |
| 面板拉手     | 顶部横条拖动指示器，负责面板上下拖拽                |
